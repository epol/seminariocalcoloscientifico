...
A common method to analyze a regularization method is to write its
filter factors.
...
\begin{mydef}
  ...
  \begin{equation}
    \label{eq:filterdef}
    x = \sum _{i=1} ^k \varphi _i \frac{1}{\sigma _i} v_i \ang{u_i,b}
  \end{equation}
\end{mydef}
...
known (for example if we know how ``smooth'' the solution should be)
setting $L_\mu = \mu L$. We will assume to not know that matrix $L$,
so we would use $L_\mu = \mu I$.
